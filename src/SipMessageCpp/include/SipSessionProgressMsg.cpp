#include "SipSessionProgressMsg.hpp"
#include "SipMessageUtil.h"
#include "SipMsgConstValue.hpp"
namespace DtSipMessageCpp
{
    CSipSessionProgressMsg::CSipSessionProgressMsg(){
    }

    CSipSessionProgressMsg::~CSipSessionProgressMsg(){}

    bool CSipSessionProgressMsg::parse(const std::string& strMsg){
        auto allLines = CProtoUtil::SplitStringByLine(strMsg, "\n");
        for (const auto& item : allLines)
        {
            //std::cout << "ITEM: " << item << std::endl;
            if (item.find(SESSION_PROGRESS_HEADER) != std::string::npos)
            {
                m_strMsgType = item;
            }
            ...
            else if (item.find(CONTACT_HEADER) != std::string::npos)
            {
                m_strContact = item;
            }
    ...
    std::string CSipSessionProgressMsg::dump() const{
