...
{
    CSipProxyAuthorizationRequiresMsg::CSipProxyAuthorizationRequiresMsg(){
    }

    CSipProxyAuthorizationRequiresMsg::~CSipProxyAuthorizationRequiresMsg(){}

    bool CSipProxyAuthorizationRequiresMsg::parse(const std::string& strMsg){
        auto allLines = CProtoUtil::SplitStringByLine(strMsg, "\n");
        for (const auto& item : allLines)
        {
            //std::cout << "ITEM: " << item << std::endl;
            if (item.find(PROXY_AUTH_REQUIRE_HEADER) !=std::string::npos)
            {
                m_strMsgType = item;
            }
    ...
    std::string CSipProxyAuthorizationRequiresMsg::dump() const{
        ...
        {
            strResult += m_strProxyAuthenticate;
            strResult += strLineEnd;
        }
